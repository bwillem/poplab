jQuery(document).ready(function($) {
 // 	var animTimer = 1000
	...
	// 	},animTimer)
	// })
	var siteURL = "http://www.poplab.io"
	...
	$(document).on('load',function(){
		$('.pop_menu').css({'z-index':'-1'});
		$('.menu-btn').trigger('click');
		setTimeout(function(){
			$('.menu-btn').trigger('click');
		},500);
		$('.pop_menu').css({'z-index':'9'});
	});
...
///////////////////////////////////////////////////////////////////////////
	...
	$.ajax({
		...
		if(Res.length){
			...
			$.each(Res, function(i,Res){
				...
				var title = Res.title.rendered
				$('.menu-item a h3').each(function(){
					if($(this).text() == title){
						$(this).parent('a').attr('data-id', Res.id).addClass('page-link');
					}
				})
				if(loadingBar = 1){
					setTimeout(function(){
	...
	var pageLoadOut = function(){
		content.removeClass('fadeInUp').removeClass('fadeInDown').addClass('fadeOutDown');
		// $('.pop_main').scrollTop(0);
		// window.setTimeout(function(){
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////
		if($('.pop_services').length){
			var whatPages = [89, 93, 94]
			$.each(whatPages, function(i, v){
				getPage(v, null);
			});
			$('.serv-menu').animate({'right':'-100px','opacity':'0'},500, function(){
				$('.serv-menu').remove();
			})
		}else{
			if($('.pop_main .serv-menu').length > 0 && $('header .serv-menu').length < 1){
				$('.serv-menu').appendTo('header').animate({'right':'35px','opacity':'1'},500);
			}else if($('.pop_main .serv-menu').length < 1){
				$('.serv-menu').animate({'right':'-100px','opacity':'0'},500, function(){
					$('.serv-menu').remove();
				})
			}
			if($('.subpage .serv-menu').length){
				$('.subpage .serv-menu').remove();
			}
		}
	...
	$('#menu-menu-1').prepend('<li><a href="/"><img class="img-center img-responsive" src="/wp-content/themes/poplab/images/pl_black-trans.png"></a></li>');
